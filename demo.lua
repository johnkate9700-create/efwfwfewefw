...
BuildTab:CreateInput({
   Name = "Select build (hut, wall, floor, big house, tower, bridge, fence, shed, platform)",
...
BuildTab:CreateButton({
   ...
   Callback = function()
      ...
      elseif _G.selectedBuild == "big house" then
         BuildBigHouse()
      elseif _G.selectedBuild == "tower" then
         BuildTower()
      elseif _G.selectedBuild == "bridge" then
         BuildBridge()
      elseif _G.selectedBuild == "fence" then
         BuildFence()
      elseif _G.selectedBuild == "shed" then
         BuildShed()
      elseif _G.selectedBuild == "platform" then
         BuildPlatform()
      else
...
RunService.RenderStepped:Connect(function()
   ...
   elseif _G.selectedBuild == "tower" then
      local center = origin + forward * 3 + Vector3.new(0, 10, 0)
      createPreviewPart(Vector3.new(6, 20, 6), CFrame.new(center, center + forward))

   elseif _G.selectedBuild == "bridge" then
      local center = origin + forward * 12 + Vector3.new(0, 1.5, 0)
      createPreviewPart(Vector3.new(4, 1, 24), CFrame.new(center, center + forward))

   elseif _G.selectedBuild == "fence" then
      local center = origin + forward * 6 + Vector3.new(0, 1.5, 0)
      createPreviewPart(Vector3.new(0.5, 3, 12), CFrame.new(center, center + forward))

   elseif _G.selectedBuild == "shed" then
      local center = origin + forward * 4 + Vector3.new(0, 2.5, 0)
      createPreviewPart(Vector3.new(8, 5, 8), CFrame.new(center, center + forward))

   elseif _G.selectedBuild == "platform" then
      local center = origin + forward * 6 + Vector3.new(0, 4, 0)
      createPreviewPart(Vector3.new(16, 0.5, 16), CFrame.new(center, center + forward))
   end
end)
...
function BuildTower()
    ...
    -- Build a tall tower (6x6 base, 20 blocks high)
    local size = 3
    for y = 0.5, 20, 0.5 do
        -- Four walls of the tower
        fire(origin + right * -size + Vector3.new(0, y, 0), origin + right * size + Vector3.new(0, y, 0))
        fire(origin + right * size + Vector3.new(0, y, 0), origin + right * size + forward * 6 + Vector3.new(0, y, 0))
        fire(origin + right * size + forward * 6 + Vector3.new(0, y, 0), origin + right * -size + forward * 6 + Vector3.new(0, y, 0))
        fire(origin + right * -size + forward * 6 + Vector3.new(0, y, 0), origin + right * -size + Vector3.new(0, y, 0))
        
        -- Add floors every 5 blocks
        if y % 5 == 0 then
            for x = -size, size - 1, 1 do
                fire(origin + right * x + forward * 0 + Vector3.new(0, y, 0), origin + right * x + forward * 6 + Vector3.new(0, y, 0))
            end
        end
    end
end
...
function BuildBridge()
    ...
    -- Build a bridge (24 blocks long, 4 wide)
    for z = 0, 24, 1 do
        -- Bridge deck
        fire(origin + right * -2 + forward * z + Vector3.new(0, 3, 0), origin + right * 2 + forward * z + Vector3.new(0, 3, 0))
        
        -- Railings
        fire(origin + right * -2 + forward * z + Vector3.new(0, 4, 0), origin + right * -2 + forward * z + Vector3.new(0, 5, 0))
        fire(origin + right * 2 + forward * z + Vector3.new(0, 4, 0), origin + right * 2 + forward * z + Vector3.new(0, 5, 0))
        
        -- Support posts every 4 blocks
        if z % 4 == 0 then
            fire(origin + right * -2 + forward * z + Vector3.new(0, 0, 0), origin + right * -2 + forward * z + Vector3.new(0, 3, 0))
            fire(origin + right * 2 + forward * z + Vector3.new(0, 0, 0), origin + right * 2 + forward * z + Vector3.new(0, 3, 0))
        end
    end
end
...
function BuildFence()
    ...
    -- Build fence posts and rails (12 blocks long)
    for z = 0, 12, 2 do
        -- Vertical posts
        fire(origin + forward * z + Vector3.new(0, 0, 0), origin + forward * z + Vector3.new(0, 3, 0))
        
        -- Horizontal rails (if not the last post)
        if z < 12 then
            fire(origin + forward * z + Vector3.new(0, 1, 0), origin + forward * (z + 2) + Vector3.new(0, 1, 0))
            fire(origin + forward * z + Vector3.new(0, 2, 0), origin + forward * (z + 2) + Vector3.new(0, 2, 0))
        end
    end
end
...
function BuildShed()
    ...
    -- Build a small shed (8x8, 5 blocks high)
    for y = 0.5, 5, 0.5 do
        -- Front wall with door opening
        if y <= 4 then
            fire(origin + right * -4 + Vector3.new(0, y, 0), origin + right * -1 + Vector3.new(0, y, 0))
            fire(origin + right * 1 + Vector3.new(0, y, 0), origin + right * 4 + Vector3.new(0, y, 0))
        else
            fire(origin + right * -4 + Vector3.new(0, y, 0), origin + right * 4 + Vector3.new(0, y, 0))
        end
        
        -- Other three walls
        fire(origin + right * 4 + Vector3.new(0, y, 0), origin + right * 4 + forward * 8 + Vector3.new(0, y, 0))
        fire(origin + right * 4 + forward * 8 + Vector3.new(0, y, 0), origin + right * -4 + forward * 8 + Vector3.new(0, y, 0))
        fire(origin + right * -4 + forward * 8 + Vector3.new(0, y, 0), origin + right * -4 + Vector3.new(0, y, 0))
    end
    
    -- Roof
    for z = 0, 8, 1 do
        fire(origin + right * -4 + forward * z + Vector3.new(0, 5.5, 0), origin + right * 4 + forward * z + Vector3.new(0, 5.5, 0))
    end
end
...
function BuildPlatform()
    ...
    -- Build support pillars first
    for x = -8, 8, 8 do
        for z = 0, 16, 8 do
            fire(origin + right * x + forward * z + Vector3.new(0, 0, 0), origin + right * x + forward * z + Vector3.new(0, 8, 0))
        end
    end
    
    -- Build the platform deck (16x16)
    for x = -8, 7, 1 do
        fire(origin + right * x + forward * 0 + Vector3.new(0, 8, 0), origin + right * x + forward * 16 + Vector3.new(0, 8, 0))
    end
    for z = 0, 16, 1 do
        fire(origin + right * -8 + forward * z + Vector3.new(0, 8, 0), origin + right * 8 + forward * z + Vector3.new(0, 8, 0))
    end
    
    -- Add railings around the platform
    for x = -8, 8, 1 do
        if x == -8 or x == 8 then
            fire(origin + right * x + forward * 0 + Vector3.new(0, 9, 0), origin + right * x + forward * 16 + Vector3.new(0, 9, 0))
        end
    end
    for z = 0, 16, 1 do
        fire(origin + right * -8 + forward * z + Vector3.new(0, 9, 0), origin + right * 8 + forward * z + Vector3.new(0, 9, 0))
    end
end
